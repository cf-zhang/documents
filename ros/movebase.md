...
* 将locus中的一些优秀的思想和工具方法引用到项目当前的导航框架中进行使用

> 优点： 项目中已有内容可延续、 优秀的工具和思想可选择

> 缺点： move_base中已知的问题无法规避

* 将move_base框架逻辑迁移到locomove_base的框架中，保留项目中已有算法级别的功能。

> 优点： 既可以使用locus提供的优秀思想，也可以将项目中已有的定制开发内容进行整合

> 缺点： nav_core 与 nav_core2之间兼容性问题， 虽然大部分功能做了适配模型。但nav_core2提供的异常处理机制的优点无法全部吸收（异常处理），需要在当前已有项目中做更改。工作量待评估

* 基于locus开源的方案进行完全二次开发，吸收项目中已有内容的优点进行重新实现

> 优点：可以完整的使用locus的优秀思想

> 缺点：项目中在使用的内容需要想locus接口进行整改，甚至好多工作都要重复进行。
